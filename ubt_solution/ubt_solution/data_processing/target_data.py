...
class TargetData:
    def __init__(self, data_dir: Path):
        ...
        try:
            ...
            # 加载倾向性目标和流行度数据
            propensity_files = [
                (data_dir / 'target' / 'propensity_category.npy', 'propensity_category'),
                (data_dir / 'target' / 'propensity_sku.npy', 'propensity_sku'),
                (data_dir / 'target' / 'popularity_propensity_category.npy', 'popularity_category'),
                (data_dir / 'target' / 'popularity_propensity_sku.npy', 'popularity_sku'),
            ]
            ...
            for file_path, attr_name in propensity_files:
                if file_path.exists():
                    setattr(self, attr_name.split('_')[-2] + '_' + attr_name.split('_')[-1], np.load(file_path))
                else:
    ...
    def get_churn_target(self, client_id: int) -> float:
        """计算流失目标值"""
        # 如果用户在目标数据中不存在，则认为是流失用户
        try:
            return 1.0 if self.train_df.loc[self.train_df["client_id"] == client_id].empty else 0.0
        except Exception as e:
    ...
    def get_category_propensity_target(self, client_id: int) -> np.ndarray:
        ...
        try:
            ...
            # 获取用户在目标数据中的品类
            cats_in_target = self.train_df.loc[self.train_df["client_id"] == client_id]["category"].unique()
    ...
    def get_product_propensity_target(self, client_id: int) -> np.ndarray:
        ...
        try:
            ...
            # 获取用户在目标数据中的产品
            skus_in_target = self.train_df.loc[self.train_df["client_id"] == client_id]["sku"].unique()
